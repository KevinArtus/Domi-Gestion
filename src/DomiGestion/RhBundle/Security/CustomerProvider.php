...
namespace DomiGestion\RhBundle\Security;

use DomiGestion\PortalBundle\Yaml\YamlRetriever;
 ...
 * @package DomiGestion\RhBundle\Security
 */
class CustomerProvider implements UserProviderInterface, UserFactoryInterface
{
    ...
    /**
     * @var \DomiGestion\PortalBundle\Yaml\YamlRetriever
     */
    ...
    public function loadUserByUsername($nom)
    {
        $entity = $this->em
            ->createQueryBuilder()
            ->select('u')
            ->from("DomiGestionRhBundle:Customer", "u")
            ->where('u.nom = :nom')
    ...
    {
        return $class === "DomiGestion/RhBundle/Security/CustomerProvider";
    }
    ...
    public function createUser($username, array $rolesStr, array $attributes)
    {
        ...
        $entity = $this->em->getRepository("DomiGestionRhBundle:Customer")->createCustomer($username, $attributes["cas:first_name"], $attributes["cas:last_name"], $attributes["cas:email"], $rolesStr, $language);
        return $entity;
    }
}
